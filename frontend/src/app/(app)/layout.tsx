...
// CHANGE: make SidebarLink consider nested routes active (e.g., /appointments/new)
function SidebarLink({ href, label }: { href: string; label: string }) {
  const pathname = usePathname();
  const active = pathname === href || pathname.startsWith(href + "/"); // CHANGE
  return (
    <Link
      href={href}
      className={[
...
export default function AppSectionLayout({ children }: { children: React.ReactNode }) {
  ...
  const [openAppointments, setOpenAppointments] = useState(true); // CHANGE: new state for appointments dropdown
    ...
    <div className="grid min-h-screen grid-cols-[16rem_1fr]">
      ...
      <aside className="border-r border-gray-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900">
        ...
        <nav className="space-y-1">
          ...
          {/* CHANGE: NEW — Appointments dropdown */}
          <div className="mt-4">
            <button
            ...
            <div
              className={`overflow-hidden transition-all duration-300 ${
                openAppointments ? "max-h-40" : "max-h-0"
              }`}
            >
              <div className="ml-4 mt-1 space-y-1 border-l border-gray-200 pl-3 dark:border-neutral-700">
